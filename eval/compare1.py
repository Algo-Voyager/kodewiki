"""AST vs naive chunking benchmark — Python-repo targeted queries.

These queries are specifically designed to expose where AST chunking outperforms
naive sliding-window chunking on Python codebases. Each query targets a concrete
AST advantage: function boundaries, class headers, docstrings, line citations.

Writes ``frontend/public/benchmark_results.json`` (gitignored).

Usage:
    python eval/compare1.py <owner>/<repo>
    python eval/compare1.py <owner>/<repo> --verbose
...
def _get_embed() -> openai.OpenAI:
    ...
    if _embed is None:
        ...
        _embed = openai.OpenAI(base_url=base_url, api_key=VLLM_API_KEY)
    return _embed
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if resp.status_code >= 500:
                print(f"    [attempt {attempt}/{retries}] Qwen 500 — "
                      f"{'retrying in 10s…' if attempt < retries else 'giving up.'}")
                if attempt < retries:
                    time.sleep(10)
                    continue
                raise RuntimeError(
                    f"Qwen service error 500 after {retries} attempts: {resp.text[:300]}"
                )
            resp.raise_for_status()
            return resp.json().get("response", "").strip()
        except httpx.TimeoutException:
            print(f"    [attempt {attempt}/{retries}] Timeout — "
...
# Queries designed to target Python AST chunking advantages.
# Each one maps to a known AST strength vs naive weakness.
BENCHMARK_QUERIES = [
    # [1] Function-level retrieval — complete function with clear signature
    "How does the pipeline generate multi-part audio files?",

    # [2] Class structure — class header + method chunks preserved separately
    "What is the role of the Pipeline class in orchestrating the workflow?",

    # [3] Function location in conditional logic — boundaries clearly marked
    "How does content preparation handle topic-based input mode?",

    # [4] Multiple functions in one module — each isolated as self-contained chunk
    "What transcription mechanisms are supported for audio processing?",

    # [5] Specific function + full docstring — complete with line numbers
    "How does the segmentation step aggregate timestamps across all parts?",

    # [6] Class methods + validation logic — header chunk + method chunks separately
    "What validation checks are performed by the PipelineConfig class?",

    # [7] Error handling function — retry logic with clear boundaries
    "How are image generation failures handled with retry logic?",

    # [8] File utility functions — fully self-contained chunks
    "What are the output directory structure and manifest hierarchy?",

    # [9] Manifest creation logic — hierarchical JSON building preserved per function
    "How does the manifest store parts, segments, and image metadata?",

    # [10] Logging utilities — log_step / setup_logger fully retrievable with docstrings
    "What are the logging utilities and how do they track pipeline metrics?",
]
...
def run_benchmark(repo_owner: str, repo_name: str, verbose: bool = False) -> dict:
    ...
    print(f"Benchmark: Python-targeted AST vs naive ({len(BENCHMARK_QUERIES)} queries)")
    if verbose:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="AST vs naive benchmark — Python-repo targeted queries"
    )
    parser.add_argument("repo", help="owner/repo  e.g. Algo-Voyager/my-python-repo")
